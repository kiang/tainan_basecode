<?php
$json = json_decode(file_get_contents(__DIR__ . '/basecode.json'), true);
...
foreach($json['features'] AS $k => $f) {
  $p = twd97_to_latlng($f['properties']['X'], $f['properties']['Y']);
  $c = new stdClass();
  $c->type = 'Feature';
  $c->properties = array(
    'weight' => intval($f['properties']['P_CNT'])
  );
  $c->geometry = array(
    'type' => 'Point',
    'coordinates' => array(
...
file_put_contents(__DIR__ . '/weight.json', json_encode($result));
